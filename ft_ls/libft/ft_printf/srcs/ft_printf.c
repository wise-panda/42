...
t_convert	ft_printf_get_flag(char c)
{
...
size_t		ft_printf_process_flag(t_print *flag, va_list *vars)
{
	size_t		count;
	t_convert	convert;

	convert = ft_printf_get_flag(flag->type);
...
int			ft_printf_scan_input(const char *format, va_list *vars)
{
	...
	if (flag == NULL)
		return (-1);
	ft_printf_initialize_flag(flag);
	while ((ret = ft_printf_assign_flag(format, flag)) != 0)
	{
		if (ret == 1)
		{
			count += write(1, format, flag->open);
			format += (flag->open + (flag->close - flag->open) + 1);
			count += ft_printf_process_flag(flag, vars);
		}
		else
			return (-1);
	}
...
int			ft_printf(const char *format, ...)
{
	...
	count = ft_printf_scan_input(format, &vars);
	va_end(vars);
	return (count);
}
